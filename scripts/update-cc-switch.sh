...
target_file="${repo_root}/modules/apps/cc-switch/package.nix"
...
asset_info="$(jq -r --arg v "${version}" '
  (
    .assets[]
    | select(.name == ("CC-Switch-" + $v + "-Linux-x86_64.deb"))
...
if [[ -z "${asset_info}" ]]; then
  asset_info="$(jq -r '
    (
      .assets[]
      | select(.name | test("(?i)linux-x86_64\\.deb$"))
      | [.browser_download_url, (.digest // "")]
      | @tsv
    ) // empty
...
if [[ "${current_version}" == "${version}" && "${current_hash}" == "${hash}" ]]; then
  echo "cc-switch is already up to date (${version})"
  exit 0
fi
