...
target_file="${repo_root}/modules/apps/netcatty/default.nix"
api_url="https://api.github.com/repos/binaricat/Netcatty/releases/latest"
...
asset_info="$(jq -r --arg v "${version}" '
  (
    .assets[]
    | select(.name == ("Netcatty-" + $v + "-linux-amd64.deb"))
...
if [[ -z "${asset_info}" ]]; then
  asset_info="$(jq -r '
    (
      .assets[]
      | select(.name | test("(?i)linux-amd64\\.deb$"))
      | [.browser_download_url, (.digest // "")]
      | @tsv
    ) // empty
...
if [[ "${current_version}" == "${version}" && "${current_hash}" == "${hash}" ]]; then
  echo "netcatty is already up to date (${version})"
  exit 0
fi

sed -Ei 's|^  version = "[^"]+";$|  version = "'"${version}"'";|' "${target_file}"
sed -Ei '0,/^    hash = "sha256-[^"]+";$/s|^    hash = "sha256-[^"]+";$|    hash = "'"${hash}"'";|' "${target_file}"
